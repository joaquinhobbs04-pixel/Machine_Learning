...
def preprocess_singlethread(trainer,list_of_lists, output_files,segs_from_prev_stage,num):
    d, _, dct = trainer.preprocess_patient(list_of_lists[num])
    print(output_files[num], dct)
    if segs_from_prev_stage is None:
        segs_from_prev_stage = [None] * len(list_of_lists)
    classes = list(range(1, trainer.num_classes))
    if segs_from_prev_stage[num] is not None:
        assert isfile(segs_from_prev_stage[num]) and segs_from_prev_stage[num].endswith(".nii.gz"), "segs_from_prev_stage" \
                                                                                                " must point to a " \
                                                                                                "segmentation file"
        seg_prev = sitk.GetArrayFromImage(sitk.ReadImage(segs_from_prev_stage[num]))
        # check to see if shapes match
        img = sitk.GetArrayFromImage(sitk.ReadImage(list_of_lists[num][0]))
        assert all([i == j for i, j in zip(seg_prev.shape, img.shape)]), "image and segmentation from previous " \
                                                                         "stage don't have the same pixel array " \
                                                                         "shape! image: %s, seg_prev: %s" % \
                                                                         (list_of_lists[num][0], segs_from_prev_stage[num])
        seg_reshaped = resize_segmentation(seg_prev, d.shape[1:], order=1, cval=0)
        seg_reshaped = to_one_hot(seg_reshaped, classes)
        d = np.vstack((d, seg_reshaped)).astype(np.float32)
    return d, output_files[num],dct
...
def predict_cases_single(model, list_of_lists, output_filenames, folds, save_npz,
                         segs_from_prev_stage=None, do_tta=True,overwrite_existing=False):
    ...
    for num in range(1):
        ...
        d,output_filename,dct = preprocess_singlethread(trainer,list_of_lists, cleaned_output_files,segs_from_prev_stage,num)
        softmax = []
        for p in params:
...
def predict_from_folder(model, input_folder, output_folder, folds, save_npz, num_threads_preprocessing,
    ...
    return predict_cases_single(model, list_of_lists[part_id::num_parts], output_files[part_id::num_parts], folds, save_npz,
                                lowres_segmentations,tta, overwrite_existing=overwrite_existing)
        #predict_cases(model, list_of_lists[part_id::num_parts], output_files[part_id::num_parts], folds, save_npz,
